...
if (isset($_SERVER['PANTHEON_ENVIRONMENT']) && $_SERVER['PANTHEON_ENVIRONMENT'] === 'live') {
  if ($_SERVER['HTTP_HOST'] === 'sr167.portoftacoma.com') {
    header('HTTP/1.0 301 Moved Permanently'); 
    header('Location: http://portoftacoma.com/news/complete-sr167'. $_SERVER['REQUEST_URI']); 
    exit();
  }
  elseif ($_SERVER['HTTP_HOST'] != 'portoftacoma.com') {
    header('HTTP/1.0 301 Moved Permanently'); 
    header('Location: http://portoftacoma.com'. $_SERVER['REQUEST_URI']); 
    exit();
  }
}
*/
...
// Redirects for legacy domain and for https://
if (isset($_SERVER['PANTHEON_ENVIRONMENT']) && $_SERVER['PANTHEON_ENVIRONMENT'] === 'live') {

  // Enable https, redirect to https-only domain
  if (
    (!isset($_SERVER['HTTP_X_SSL']) || (isset($_SERVER['HTTP_X_SSL']) && $_SERVER['HTTP_X_SSL'] != 'ON')) 
    //&& (strpos($_SERVER['REQUEST_URI'], 'cart') !== FALSE || strpos($_SERVER['REQUEST_URI'], 'checkout') !== FALSE)
  ) {
    header('HTTP/1.0 301 Moved Permanently');
    header('Location: https://www.charities.org'. $_SERVER['REQUEST_URI']);
    $conf['https'] = TRUE;
    exit();
  }
  

  if ($_SERVER['HTTP_HOST'] === 'membership.charities.org') {
    header('HTTP/1.0 301 Moved Permanently'); 
    header('Location: https://www.charities.org'. $_SERVER['REQUEST_URI']); 
    exit();
  }
  //elseif (!function_exists('drush_main') && ($_SERVER['HTTP_HOST'] != 'www.charities.org' || empty($_SERVER['HTTPS']))) {
  //  header('HTTP/1.0 301 Moved Permanently'); 
  //  header('Location: https://www.charities.org'. $_SERVER['REQUEST_URI']); 
  //  exit();
  //}
